...
{
    addedPhotos     : string[];
    onChangePhotos  : React.Dispatch<React.SetStateAction<string[]>>;
    };

const PhotoUploader = ( {addedPhotos, onChangePhotos} : Props ) => 
{
    ...
    function removePhoto(ev : React.MouseEvent<HTMLButtonElement>, filename : string)
    {
        ...
        onChangePhotos([...addedPhotos.filter((photo : string) => photo !== filename)]);
    }

    function setAsMainPhoto(ev : React.MouseEvent<HTMLButtonElement>, filename : string)
    {
        ev.preventDefault();
        const photos = [filename, ...addedPhotos.filter((photo : string) => photo !== filename)];
        // const photos = Array<string>(/* addedPhotos.length */); // need a extra index if pre-allocate, no reserve()...
                ...
                { addedPhotos && addedPhotos.length > 0 && addedPhotos.map(
                    ...
                    (
                        <div key = {link} className = "h-32 flex relative">
                            <img className = "rounded-2xl w-full object-cover" src = {`${serverUploadPhotoUrl}/${link}`}></img>
